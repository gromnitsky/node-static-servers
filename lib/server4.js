...
export default function(req, writable, name, opt = {}) {
    ...
    fs.stat(file, (err, stats) => {
        ...
        if (rr.ranges?.length > 1) {
            rr.ranges.forEach( (range, idx) => {
                rr.multipart_write(range, idx)
            })
        } else {
            let ro = {}
...
class RangeRequest {
    constructor(readable, writable, opt, file, stats, headers) {
        ...
        this.ranges = readable.headers.range?.match(/bytes=(\S+)/)?.[1]
        let if_range = readable.headers['if-range']
        if (if_range) {
            let ims = new Date(if_range).getTime()
            if (ims >= stats.mtime.toUTCString()
                || readable.headers['if-range'] !== etag(stats, headers)) {
                this.ranges = null
            }
        }
        if (this.ranges) this.mk_headers()
    }

    mk_headers() {
        if ( !(this.ranges = range_parse(this.ranges, this.stats.size)))
            throw ECHRNG
    ...
    multipart_write(range, idx) {
        ...
        readable.on('error', err => error(this.writable, err, this.opt.verbose))
        readable.once('data', () => {
    ...
    boundary() {
        return this._boundary
            || (this._boundary = crypto.randomBytes(10).toString('hex'))
    }
    ...
    content_range(range) {
        return `${range[0]}-${range[1]}/${this.stats.size}`
    }
}
