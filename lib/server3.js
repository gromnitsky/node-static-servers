...
export default function(req, writable, name, opt = {}) {
    ...
    fs.stat(file, (err, stats) => {
        ...
        let compressor; try {
            compressor = content_encoding(req, writable, file, opt)
        } catch (err) {
            return error(writable, err, opt.verbose) // 406
        }
        ...
        let streams = [readable, compressor, writable].filter(Boolean)
...
export function accept_encoding_negotiate(algo, enc) {
    let err = new Error('Pas acceptable'); err.code = 'EBADE'

    let v = enc.find( v => v.name === algo)
    let asterisk = enc.find( v => v.name === '*')
        || enc.find( v => v.name === 'identity')
    if (!v && asterisk?.q === 0) throw err
    if (v && v.q === 0) throw err
    if (!v) return false
